# Serial Mediation Models Corrected for Measurement Error
...
mediators, `fm1` and `fm3`, with `fc1` and `c2` as the control
variables.
...
effect along the path `fx -> fm1 -> fm3 -> fy`, as well as two other
paths, `fx -> fm1 -> y` and `fx -> fm3 -> y`, using nonparametric
bootstrapping.
...
- `m`: A vector of the names of the mediators (“m” variables). The order
  ...
  name to the last name. Therefore, if the path is
  `x -> fm1 -> fm3 -> y`, set this to `c("fm1", "fm3")`.
