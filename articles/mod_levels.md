...
levels for `w1`, one standard deviation (SD) below the mean, the mean,
and one SD above the mean, just call
[`mod_levels()`](https://sfcheung.github.io/manymome/reference/mod_levels.md):
...
This is not necessary when calling
[`cond_indirect_effects()`](https://sfcheung.github.io/manymome/reference/cond_indirect.md)
...
the mean, we can set the argument `sd_from_mean` to a vector of
distances from the mean, which are `-1` and `1` in this example:
...
SD below the mean, the mean, and one SD above the mean in a normal
distribution.
...
the mean and one SD above the mean.
...
To tell `mod_levels` which part to remove, set `prefix` to the part to
be removed:
...
By default, the group with 0s on all dummy variables is labeled
...
To have full control over the labels and coding, use `values` and supply
a *named* *list* of numeric vectors: the *name* is the group label and
the *vector* is the coding:
...
is wrong or users want to specify the type explicitly, set the argument
