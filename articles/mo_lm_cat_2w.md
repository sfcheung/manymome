...
The moderator `gp` has two possible values: `"Control"` and
`"Treatment"`.

The moderator `site` has three possible values: `"Site 1"`, `"Site 2"`,
and `"Site 3"`.

We will first consider two models, each with only one of the two
moderators.
...
We can now use
[`cond_effects()`](https://sfcheung.github.io/manymome/reference/cond_indirect.md)
to estimate the effects of `x` on `y` for different categories of the
moderator `gp`.
...
(gp)](mo_lm_cat_2w_plot_cond_gp-1.png)

Conventional Plot of Conditional Effects (gp)
...
(gp)](mo_lm_cat_2w_plot_cond_gp_tumble-1.png)

Tumble Plot of Conditional Effects (gp)

In this example, the distributions of `x` for the groups are similar.
...
The standardized conditional effects from `x` to `y` conditional on `gp`
can be estimated by setting `standardized_x` and `standardized_y` to
`TRUE`.
...
![Conventional Plot of Standardized Conditional Effects
(gp)](mo_lm_cat_2w_plot_std_gp-1.png)

Conventional Plot of Standardized Conditional Effects (gp)
...
![Tumble Plot of Standardized Conditional Effects
(gp)](mo_lm_cat_2w_plot_std_gp_tumble-1.png)

Tumble Plot of Standardized Conditional Effects (gp)

## One Categorical Moderator: Three Categories

The steps demonstrated above can be used for a categorical moderator
...
The function
[`cond_effects()`](https://sfcheung.github.io/manymome/reference/cond_indirect.md)
will determine the number of categories automatically:
...
![Conventional Plot of Conditional Effects
(site)](mo_lm_cat_2w_plot_cond_site-1.png)

Conventional Plot of Conditional Effects (site)
...
![Tumble Plot of Conditional Effects
(site)](mo_lm_cat_2w_plot_cond_site_tumble-1.png)

Tumble Plot of Conditional Effects (site)

This example demonstrates the advantage of the tumble graph. The
distribution of `x` in `"Site 3"` has a mean lower than those in the
other two sites. The conventional plot using the same range of `x` in
all three groups will give the incorrect impression of a cross-over of
the lines in the *samples* (though it is possible that such a cross-over
of the lines happens in the *populations*).
...
These are the plots of the standardized conditional effects:
...
![Conventional Plot of Standardized Conditional Effects
(site)](mo_lm_cat_2w_plot_std_site-1.png)

Conventional Plot of Standardized Conditional Effects (site)
...
![Tumble Plot of Standardized Conditional Effects
(site)](mo_lm_cat_2w_plot_std_site_tumble-1.png)

Tumble Plot of Standardized Conditional Effects (site)

## Two Categorical Moderators

The steps demonstrated above can be used in a regression model with any
number of moderators.

Suppose this is the model being fitted, with control variables omitted
from the plot for readability, both `gp` and `site` included, but no
...
The function
[`cond_effects()`](https://sfcheung.github.io/manymome/reference/cond_indirect.md)
can be used for any number of moderators, as long as they are listed in
`wlevels`:
...
![Conventional Plot of Conditional Effects (gp +
site)](mo_lm_cat_2w_plot_cond_gp_site-1.png)

Conventional Plot of Conditional Effects (gp + site)
...
moderator for each category of the other moderator.

This is the plot of effects for `"Control"` and `"Treatment"`:
...
![Conventional Plot of Conditional Effects: Site within
Group](mo_lm_cat_2w_plot_cond_gp_site_cols_gp-1.png)

Conventional Plot of Conditional Effects: Site within Group
...
![Conventional Plot of Conditional Effects: Group within
Site](mo_lm_cat_2w_plot_cond_gp_site_cols_site-1.png)

Conventional Plot of Conditional Effects: Group within Site

Note that, without three-way interaction, the *moderating effect* of
`gp` is the same in all three sites, and the *moderating effect* of
`site` is the same in all two groups. The six lines are different simply
because the effect of `x` depends on *both* `gp` and `site`. They do
*not* denote three-way interaction (because it is not in the regression
model).

#### Tumble Plots

We already know the distributions of `x` are not the same in all three
sites. Therefore, the tumble graph is a better way to visualize the
effects:
...
![Tumble Plot of Conditional Effects: Group within
Site](mo_lm_cat_2w_plot_cond_gp_site_cols_gp_tumble-1.png)

Tumble Plot of Conditional Effects: Group within Site
...
![Tumble Plot of Conditional Effects: Site within
Group](mo_lm_cat_2w_plot_cond_gp_site_cols_site_tumble-1.png)

Tumble Plot of Conditional Effects: Site within Group
...
![Tumble Plot of Standardized Conditional Effects: Site within
Group](mo_lm_cat_2w_plot_std_gp_site_tumble_cols_gp-1.png)

Tumble Plot of Standardized Conditional Effects: Site within Group
...
![Tumble Plot of Standardized Conditional Effects: Group within
Site](mo_lm_cat_2w_plot_std_gp_site_tumble_cols_site-1.png)

Tumble Plot of Standardized Conditional Effects: Group within Site
...
with each other. That is, the group difference in the effect of `x` may
not be the same in all three sites, or the site difference in the effect
of `x` may not be the same in the two groups.
...
The test of the difference between this model and the previous model
with no three-way interaction supports that this model fits better:
...
![Tumble Plot of Conditional Effects: Site within
Group](mo_lm_cat_2w_plot_cond_gp_x_site_cols_gp-1.png)
...
![Tumble Plot of Conditional Effects: Group within
Site](mo_lm_cat_2w_plot_cond_gp_x_site_cols_site-1.png)
...
![Tumble Plot of Standardized Conditional Effects: Site within
Group](mo_lm_cat_2w_plot_std_gp_x_site_tumble_cols_gp-1.png)
...
![Tumble Plot of Standardized Conditional Effects: Group within
Site](mo_lm_cat_2w_plot_std_gp_x_site_tumble_cols_site-1.png)
