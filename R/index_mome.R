...
#'   Moderated Mediation
#'
#' @description It computes the index of moderated mediation and
#'   the index of moderated moderated mediation proposed by
#'   Hayes (2015, 2018).
#'
#' @details
#' The function [index_of_mome()] computes the
#' *index of moderated mediation* proposed by Hayes (2015).
#' It supports any path
#' in a model with one (and only one) component path moderated.
#' For example, `x->m1->m2->y` with `x->m1` moderated by `w`.
#' It measures the change in indirect effect when the moderator
#' increases by one unit.
#'
#' The function [index_of_momome()] computes the
#' *index of moderated moderated mediation* proposed by Hayes (2018).
#' It supports any path
#' in a model, with two component paths moderated,
#' each by one moderator.
#' For example, `x->m1->m2->y` with `x->m1` moderated by `w`
#' and `m2->y` moderated by `z`.
#' It measures the change in the index of moderated mediation
#' of one moderator when the other moderator increases by one unit.
#'
#' @return
...
#' for extracting the bootstrap confidence interval if available
...
#'   [lavaan::lavaan-class] object, a list of [lm()] outputs,
#'   or an object created by [lm2list()].
