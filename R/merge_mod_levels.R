...
#'
#' @description Merge the levels of moderators generated by
#'  [mod_levels()] into a data frame.
#'
#' @details It merges the levels of moderators generated by
#'  [mod_levels()] into a data frame, with each row represents a
#'  combination of the levels. The output is to be used by
#'  [cond_indirect_effects()].
#'
#' Users usually do not need to use this function because
#'  [cond_indirect_effects()] will merge the levels
#'  internally if necessary. This function is used
#' when users need to customize the levels for each moderator
#' and so cannot use [mod_levels_list()] or the default
#' levels in [cond_indirect_effects()].
#'
#' @return A `wlevels`-class object, which is
#' a data frame of the combinations of levels,
#' with additional attributes about the
#' levels.
#'
#' @param ... The output from [mod_levels()], or a list
#'  of levels generated by [mod_levels_list()].
#'
#'
#' @seealso [mod_levels()] on generating the levels of a moderator.
#'
#' @examples
#'
