...
indirect_to_note <- function(
  ...
  m <- ind_out$m
  if (is.list(m)) {
    s_path <- paste(x, total_symbol, y)
  } else {
    s_path <- paste(c(x, m, y), collapse = " -> ")
  }
...
text_indirect_list <- function(
  ...
  paths <- names(object)
  if (pad_ws) {
    nchar_max <- max(sapply(paths, nchar))
    out0 <- sapply(
              paths,
              function(x) {
                paste0(x, strrep(" ", nchar_max - nchar(x)), ":")
              })
  } else {
    out0 <- paste0(paths, ":")
  }
  out1 <- paste(out0, object)
  out1
}
...
# Output:
# - A character vector to be printed.
text_indirect <- function(
                    object,
                    prefix = "Total indirect effect: ") {
  path <- names(object)
  out0 <- paste0(path, ": ")
  out1 <- paste0(prefix, out0, object)
  out1
}
