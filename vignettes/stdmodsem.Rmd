...
if (file.exists(fn)) {
    ...
    fit_lm_boot_out <- lm2boot_out(fit_lm, R = 500, seed = 87415)
    saveRDS(fit_lm_boot_out, fn)
  }
```

(Note: Only 500 bootstrap samples are used for illustration. Set
R to 2000 or even 5000 in real research.)
...
out_lm
tmp <- cond_indirect(wvalues = unlist(mod_levels[1, ]),
                    x = "x",
                    y = "y",
                    m = "m",
                    fit = fit_lm,
                    boot_ci = TRUE,
                    boot_out = fit_lm_boot_out)
tmp2 <- cond_indirect(wvalues = unlist(mod_levels[1, ]),
                    x = "x",
                    y = "y",
                    m = "m",
                    fit = fit_lm)
tmpfct <- function(d, i) {
    lm_m <- lm(m ~ x*w1, d[i, ])
    lm_y <- lm(y ~ m*w2 + x, d[i, ])
    out <- cond_indirect(wvalues = c(w1 = 6.17315699477943, w2 = 4.04048736536153),
                  x = "x", y = "y", m = "m", fit = lm2list(lm_m, lm_y))
    return(coef(out))
  }
library(boot)
bout <- boot(dat, tmpfct, R = 2000)
```
...
```{r}
library(lavaan)
dat$x_w1 <- dat$x * dat$w1
dat$m_w2 <- dat$m * dat$w2
mod <-
"
m ~ x + w1 + x_w1
y ~ m + w2 + m_w2 + x
...
fit_lavaan <- sem(model = mod,
                  data = dat,
                  fixed.x = FALSE)
parameterEstimates(fit_lavaan)[parameterEstimates(fit_lavaan)$op == "~", ]
```
...
"
fit0 <- sem(mod, dat, do.fit = FALSE, fixed.x = FALSE)
layout_m <- matrix(c("x1", NA, NA, NA,
                     NA, "m11", "m12", "y1",
                     "x2", NA, NA, NA), 3, 4, byrow = TRUE)
p <- semPaths(fit0,
              layout = layout_m,
              nCharNodes = 0,
              exoCov = TRUE,
              ...
              # curve = rep(2, 15),
              DoNotPlot = TRUE)
p$graphAttributes$Edges$curve[10:11] <- c(-2, 2)
plot(p)
```
...
```{r}
lm_m11 <- lm(m11 ~ x1 + x2, dat)
lm_m12 <- lm(m12 ~ m11 + x1 + x2, dat)
...
```{r}
fit_lm <- lm2list(lm_m11, lm_m12, lm_y1)
fit_lm
```
...
```{r eval = TRUE}
fit_lm_boot_out_complicated <- lm2boot_out(fit_lm, R = 500, seed = 87415)
```
...
```{r}
out_x1y1_lm <- cond_indirect(x = "x1",
                        y = "y1",
                        m = c("m11", "m12"),
                        fit = fit_lm,
                        boot_ci = TRUE,
...
The indirect effect in the pathway `x2 -> m12 -> m12 -> y1` can be
estimated similarly:

```{r}
out_x2y1_lm <- cond_indirect(x = "x2",
                        y = "y1",
                        m = c("m11", "m12"),
                        fit = fit_lm,
                        boot_ci = TRUE,
...
to `y1` through `m12` only, bypassing `m11`. This can be done
by setting `m` only to `m12`:

```{r}
out_x1y1_m12_lm <- cond_indirect(x = "x1",
                        y = "y1",
                        m = "m12",
                        fit = fit_lm,
                        boot_ci = TRUE,
...
```{r}
out_m11y1_lm <- cond_indirect(x = "m11",
                        y = "y1",
                        m = "m12",
                        fit = fit_lm,
                        boot_ci = TRUE,
...
The standardized solution can be computed by setting `standardized_x`
and `standardized_y` to `TRUE`:

```{r}
out_x1y1_lm_std <- cond_indirect(x = "x1",
                        y = "y1",
                        m = c("m11", "m12"),
                        fit = fit_lm,
                        boot_ci = TRUE,
...
```{r}
out_x1y1_lavaan <- cond_indirect(x = "x1",
                        y = "y1",
                        m = c("m11", "m12"),
                        fit = fit_lavaan,
                        boot_ci = TRUE,
...
```{r}
out_x2y1_lavaan <- cond_indirect(x = "x2",
                        y = "y1",
                        m = c("m11", "m12"),
                        fit = fit_lavaan,
                        boot_ci = TRUE,
...
```{r}
out_x1y1_m12_lavaan <- cond_indirect(x = "x1",
                        y = "y1",
                        m = "m12",
                        fit = fit_lavaan,
                        boot_ci = TRUE,
...
```{r}
out_m11y1_lavaan <- cond_indirect(x = "m11",
                        y = "y1",
                        m = "m12",
                        fit = fit_lavaan,
                        boot_ci = TRUE,
...
```{r}
out_x1y1_lavaan_std <- cond_indirect(x = "x1",
                        y = "y1",
                        m = c("m11", "m12"),
                        fit = fit_lavaan,
                        boot_ci = TRUE,
