...
to be conducted once. The bootstrap estimates can be reused by
most main functions of `manymome` for any path and any levels
of the moderators.
...
- Use `indirect_effect()` to compute *indirect* *effects*,
...
`lavaan::sem()`. There is not need to
...
sampled from the populations, and so their SDs are sample
statistics.
...
to be done. There are several to do this. We first illustrate
using `do_boot()`.
...
estimates as well as implied variances and SDs of variables for
...
- `ncores`: The number of processes in parallel processing.
   ...
   minus 1. Can be omitted in real studies. Set to 2 here for
   illustration.
...
of the moderators can be used. See `mod_levels()` and `merge_mod_levels()`
...
in a model
that starts with any variable, ends with
any variable, through any variable(s).
...
and `c2` are labelled as control variables, if appropriated,
...
The package `manymome` has these advantages:

- The model fitting step is easier. No need to label
...
- Bootstrapping only needs to be done once. The bootstrap
  ...
  useful when the sample size and there is missing data.
...
need to be used for these cases.
