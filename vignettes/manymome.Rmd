...
can be
used in some typical cases. It assumes that readers have a
...
- Stage 1: **Fit the model**
    ...
    - This can be done by SEM (using `lavaan::sem()`) or a series of regressions
  (using `lm()`).
...
intervals for the effects. Bootstrapping can be done in Stage 1
(e.g., by `lavaan::sem()` using `se = "boot"`) or in Stage 2
in the first call to the main functions, and only needs
to be conducted once. Alternatively, `do_boot()` can be used
...
methods for standard errors and confidence intervals for other parameters,
...
- `boot_out`: The pregenerated bootstrap estimates generated
              by `do_boot()` or a previous call to
...
above their means. The indirect effect at these levels of `w1` and `w2` is
0.399, with
...
For example, this shows the details of the computation of the
...
be estimated, along with a bootstrap confidence interval, using
...
does not contradict the significant moderation effect
...
Because a bootstrap confidence interval is requested, the
...
and `c2` are labeled as control variables, if appropriate,
...
- The model fitting stage is easier. No need to label
  ...
  use other methods for confidence intervals and use
  bootstrapping only for indirect effects and conditional
  indirect effects.

- Missing data can be handled by `missing = "fiml"`
  in calling
...
- Missing data handled by multiple imputation
  ...
  Monte Carlo confidence intervals can be formed for
