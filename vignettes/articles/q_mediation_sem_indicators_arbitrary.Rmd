...
to one form of model. See a full list
...
- `y_1`, `y_2`, `y_3`, and `y_4`:
  ...
  The indicator `y_4` is a reverse item.
...
- `model`: A character vector of the paths
  in the model. Each path is of this form:
...
especially if missing data is present,
...
- The reliability coefficient for each
  ...
  The omega coefficient is used, computed by
  `semTools::compRelSEM()`.

## Model Fit

The section `Structural Equation Modeling Results`
prints the results from `lavaan` for the model.
...
the quick mediation functions has
arguments for customizing the output.
These are arguments that may be
used:
...
set `parallel` to `FALSE`. It will take
longer, sometimes much longer, to run
