...
The main function, `get_prod()`, is an internal
function not exported. Nevertheless, advanced users may be
interested in learning how it works.

# `get_prod()`

This internal function is used to find all product term(s), if any,
...
```{r}
manymome:::get_prod(x = "x", y = "m1", fit = fit)
```

Case: A path with more than one moderator

```{r}
manymome:::get_prod(x = "x", y = "y", fit = fit)
```

Case: A path without a moderator

```{r}
manymome:::get_prod(x = "m2", y = "m3", fit = fit)
```

Case: `x` does not have a direct path to `y`

```{r}
manymome:::get_prod(x = "m1", y = "m3", fit = fit)
```
...
```{r}
manymome:::get_prod(x = "x", y = "m1", est = lm_est$est, data = lm_est$data)
```

Case: A path with more than one moderator

```{r}
manymome:::get_prod(x = "x", y = "y", est = lm_est$est, data = lm_est$data)
```

Case: A path without a moderator

```{r}
manymome:::get_prod(x = "m2", y = "m3", est = lm_est$est, data = lm_est$data)
```

Case: `x` does not have a direct path to `y`

```{r}
manymome:::get_prod(x = "m1", y = "m3", est = lm_est$est, data = lm_est$data)
```
...
```{r}
manymome:::get_prod(x = "x", y = "m1", est = lm_est$est, operator = ":")
```

Case: A path with more than one moderator

```{r}
manymome:::get_prod(x = "x", y = "y", est = lm_est$est, operator = ":")
```

Case: A path without a moderator

```{r}
manymome:::get_prod(x = "m2", y = "m3", est = lm_est$est, operator = ":")
```

Case: `x` does not have a direct path to `y`

```{r}
manymome:::get_prod(x = "m1", y = "m3", est = lm_est$est, operator = ":")
```
