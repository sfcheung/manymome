...
to multigroup models. Readers are assumed to have a basic
understanding of using `manymome`.
Please refer to
...
for an illustration of this function).
...
The function `indirect_effect()` can be used as usual
...
When used on a multigroup model,
we can omit the argument `wlevels`.
...
Though it may not sound intuitive, the printout
...
Note that, unlike a single-group model, in multigroup models,
the standardized indirect effect in a group uses
...
known phenomenon in multigroup structural equation modeling.

The difference in the two completely standardized indirect
effects can be computed and tested using the math operator `-`:
...
For example, the following identifies all paths from `x1`
to `y1`:
...
Though the focus is on indirect effects,
...
using bootstrapping confidence intervals using
`cond_indirect_diff()`:
...
Note: The argument `keep_wlevels_order` needs to
set to `FALSE` for version 0.3.4 or above to
keep the order of the grouping variable in the
plot. It only affects the order of group labels
in the plot and the default colors.
...
constraints are imposed, these means and SDs are
...
It would be misleading if the two lines are plotted at the
...
users reporting issues discovered when
